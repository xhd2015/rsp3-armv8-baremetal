...
public:
	...
	AS_MACRO void writeData(char ch)	{		reg<char,UARTDR>() = ch;	}
	AS_MACRO uint16_t readData()const	{		return reg<uint16_t,UARTDR>();	}
	...
	AS_MACRO bool hasReceiveData()const { return bitsNonSet<3,4>(reg16<UARTFR>());}// no-busy,not-empty
	AS_MACRO void enable(bool enable){ setBit(reg16<UARTCR>(),0,enable);}
	AS_MACRO void enableReceive(bool enable){ setBit(reg16<UARTCR>(),9,enable);}
	AS_MACRO void enableSend(bool enable){ setBit(reg16<UARTCR>(),8,enable);}
	AS_MACRO void readInterruptLevel(Level level) {setBits(reg16<UARTIFLS>(),3,5,level);}
	AS_MACRO void enableReceiveInterrupt(bool enable) { setBit(reg16<UARTIMSC>(), 4,enable);}
