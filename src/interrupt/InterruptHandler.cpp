...
void TEMPLATED_InterruptHandler::handleSVC(SvcFunc func)
{
	...
	case SvcFunc::puts:
	{
		...
		auto printkChars = terminalOut.print(str, len);
	...
	case SvcFunc::ps:
	{
		processManager.printProcessInformation(terminalOut);
		break;
	}
	case SvcFunc::fork:
	{
...
//		_intm->enableInterrupt(id,false);
		...
		// UPDATE 2018年6月3日01:01:20 使用后一种方式现在也是正确的（对于一般命令而言）
...
//	while(pl011.readReady() && !inputBuffer.full())
//	{
////		auto ch=pl011.rawRead();
//		inputBuffer.put(pl011.rawRead());
////		inputBuffer.put(ch);
////		kout << Hex(ch) << "\n";
//	}
	fillInputBuffer();
...
TEMPLATE_InterruptHandler
void TEMPLATED_InterruptHandler::handleSysCallGets()
{
	auto cur = processManager.currentRunningProcess();
	auto r   = currentState().registers();
		...
		if(buff && buff->empty()) // 为了安全buff必须为空，因为buff不为空时，数据可能正在被操作
		{
			...
			if(inputBuffer.empty())
			{
				if(block) // 阻塞
				{
					...
					r[0]=0;//设置阻塞进程唤醒后的返回值,需要它重新请求一次
					cur->data<true>().saveContext(r);
					schedule();
				}
			}else{
...
TEMPLATE_InterruptHandler
void TEMPLATED_InterruptHandler::handleSysCallKillProcess()
{
	kout << INFO << "kill process [ pid = ";
	// 收回资源： 占用的内存，占用的pid，打开的文件等， 将其从进程队列中清除
	auto pid = static_cast<Pid>(currentState().registers()[0]);
	auto exitCode = static_cast<int>(currentState().registers()[1]);
	auto proc = processManager.findAliveProcess(pid);
	if(proc)
	{
		...
		proc->data<true>().exitCode(exitCode);
		processManager.killProcess(proc);
		schedule();
	}
}
TEMPLATE_InterruptHandler
void TEMPLATED_InterruptHandler::fillInputBuffer()
{
	while(pl011.readReady() && !inputBuffer.full())
	{
//		auto ch=pl011.rawRead();
